...
const enhancedValidation_1 = require("./services/oemResolver/enhancedValidation");
...
╚════════════════════════════════════════════════════════════════════════════╝
    ...
    for (let i = 0; i < TEST_CASES.length; i++) {
        ...
        try {
            ...
            stepLog.push('------------------------');
            const oemResult = await (0, oemResolver_1.resolveOEM)(req);
            ...
            if (oemResult.candidates.length > 0) {
                const uniqueSources = new Set(oemResult.candidates.map((c) => c.source.split('+')[0]));
            ...
            const validation = await (0, enhancedValidation_1.performEnhancedValidation)(oemResult.primaryOEM, oemResult.candidates, testCase.vehicle.make, testCase.vehicle.model, testCase.part.rawText, backsearchResult, {
    ...
    // Write to file
    const fs = require('fs');
    const logPath = path_1.default.join(__dirname, `enhanced_validation_test_${Date.now()}.log`);
