...
jest.mock('./supabaseService', () => ({
    ...
    insertShopOffers: jest.fn(async (orderId, oem, offers) => {
    ...
    listShopOffersByOrderId: jest.fn(async (orderId) => mockInsertedOffers.filter((o) => o.orderId === orderId))
}));
const botLogicService_1 = require("./botLogicService");
const openAiService_1 = require("./openAiService");
const oemService_1 = require("./oemService");
...
describe('E2E mocked: WhatsApp -> Orchestrator -> OEM -> Scrape -> Dashboard', () => {
    ...
    it('processes message, resolves OEM, scrapes offers and returns dashboard-listable offers', async () => {
        const payload = { from: 'whatsapp:+49123456789', text: 'Bremsscheiben vorne', mediaUrls: [] };
        ...
        expect(oemService_1.resolveOEM).toHaveBeenCalled();
        ...
        expect(supabaseService_1.insertShopOffers).toHaveBeenCalled();
        // listShopOffersByOrderId should return the inserted offers
        const offers = await (0, supabaseService_1.listShopOffersByOrderId)('order-e2e');
