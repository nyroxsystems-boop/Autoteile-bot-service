...
exports.RealisticBrowserScraper = void 0;
...
class RealisticBrowserScraper {
    ...
    async fetchOffers(oem) {
        logger_1.logger.info(`[${this.name}-Realistic] Scraping with realistic browser for ${oem}...`);
        try {
            const context = await this.getBrowserContext();
            const page = await context.newPage();
            const url = this.buildTargetUrl(oem);
            logger_1.logger.info(`[${this.name}-Realistic] Navigating to: ${url}`);
            ...
            if (this.targetShop === 'kfzteile24') {
                ...
                if (currentUrl.includes('/ersatzteile-') || currentUrl.includes('/bremsanlage')) {
                    logger_1.logger.info(`[${this.name}-Realistic] Detected category redirect, trying alternative approach...`);
                    ...
                    try {
                        ...
                        if (await categoryLink.count() > 0) {
                            logger_1.logger.info(`[${this.name}-Realistic] Clicking on Bremsscheiben category...`);
                        ...
                        logger_1.logger.warn(`[${this.name}-Realistic] Could not navigate to category`);
                    }
                }
            }
            ...
            for (const selector of productSelectors) {
                try {
                    ...
                    logger_1.logger.info(`[${this.name}-Realistic] Products loaded with selector: ${selector}`);
                    productsLoaded = true;
                    break;
                }
                catch (e) {
            ...
            if (!productsLoaded) {
                logger_1.logger.warn(`[${this.name}-Realistic] No products found with any selector`);
            }
            ...
            logger_1.logger.info(`[${this.name}-Realistic] Saved debug files to ${debugDir}`);
            await page.close();
            logger_1.logger.info(`[${this.name}-Realistic] Got HTML (${html.length} bytes)`);
            return this.parseHtml(html, oem, url);
        }
        catch (error) {
            logger_1.logger.error(`[${this.name}-Realistic] Error`, { error: error.message });
            return [];
        }
    }
    async humanWait(min, max) {
    ...
    parseHtml(html, oem, url) {
        ...
        if (this.targetShop === 'autodoc') {
            ...
            if (items.length > 0) {
                logger_1.logger.info(`[${this.name}-Realistic] Found ${items.length} Autodoc products`);
                items.slice(0, 10).each((i, el) => {
                    try {
                        ...
                        logger_1.logger.warn(`[${this.name}-Realistic] Error parsing item: ${e}`);
                    }
                });
            }
            else {
                logger_1.logger.warn(`[${this.name}-Realistic] No .listing-item elements found`);
            }
        }
        else {
            // KFZTeile24 parsing...
            ...
            for (const selector of strategies) {
                ...
                if (items.length > 0) {
                    logger_1.logger.info(`[${this.name}-Realistic] Found ${items.length} items`);
                    items.slice(0, 10).each((i, el) => {
                        try {
                            const $el = $(el);
...
exports.RealisticBrowserScraper = RealisticBrowserScraper;
