...
jest.mock('./scrapingService', () => ({
    scrapeOffersForOrder: jest.fn(async (orderId, oemNumber) => ({ ok: true, offersInserted: 3 }))
}));
...
describe('integration: orchestrator -> OEM -> scrape', () => {
    ...
    it('calls resolveOEM and scrapeOffersForOrder when orchestrator returns oem_lookup', async () => {
        const payload = { from: 'user-int', text: 'Ich brauche Bremsscheiben', mediaUrls: [] };
        ...
        expect(oemService_1.resolveOEM).toHaveBeenCalledWith(expect.objectContaining({ make: 'VW', model: 'Golf', year: 2015, vin: 'WVWZZZ1JZXW000001' }), expect.stringContaining('Bremsscheiben'));
        // Scraping should be started with returned OEM
        expect(scrapingService_1.scrapeOffersForOrder).toHaveBeenCalledWith('order-int-1', 'OEM-INT-123');
