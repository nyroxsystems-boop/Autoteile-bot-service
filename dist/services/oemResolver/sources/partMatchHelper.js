...
exports.resolveAftermarketToOEM = resolveAftermarketToOEM;
exports.filterByPartMatch = filterByPartMatch;
 ...
 */
async function resolveAftermarketToOEM(req) {
    const prompt = `
    ...
    `;
    try {
        const completion = await openAiService_1.client.chat.completions.create({
...
async function filterByPartMatch(candidates, req) {
  ...
  `;
    try {
        ...
        const validOems = new Set(Array.isArray(parsed) ? parsed : (parsed.oems || parsed.valid_oems || Object.values(parsed)[0] || []));
        if (validOems.size === 0)
            return candidates; // Conservative fallback
