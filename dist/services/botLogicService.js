...
const oemService = __importStar(require("./oemService"));
...
async function callOrchestrator(payload) {
    try {
        ...
        logger_1.logger.debug?.("Orchestrator raw response", { raw, short: (typeof raw === 'string' ? raw.slice(0, 200) : raw) });
        const start = raw.indexOf("{");
        const end = raw.lastIndexOf("}");
        const jsonString = start !== -1 && end !== -1 && end > start ? raw.slice(start, end + 1) : raw;
        const parsed = JSON.parse(jsonString);
        ...
        logger_1.logger.warn("Orchestrator call failed or returned invalid JSON", { error: err?.message });
        return null;
    }
}
...
`;
    try {
        ...
        logger_1.logger.error("OpenAI Vision OCR failed", { error: err?.message });
        return {
            make: null,
            model: null,
            vin: null,
...
Extrahiere neue Infos aus der Nachricht. Überschreibe bekannte Felder nur, wenn der Nutzer sie explizit korrigiert.`;
    try {
        ...
        logger_1.logger.error("OpenAI text understanding failed", { error: err?.message });
        return {
            intent: "OTHER",
            requestedPart: null,
            vehiclePatch: {},
    ...
    return withConversationLock(payload.from, async () => {
        ...
        if (intent === "status_question") {
            ...
            if (language === "en") {
                ...
                if (status === "done")
                    statusReply += "Deine Bestellung ist abgeschlossen und sollte bald bei dir sein!";
                else if (status === "ready")
                    statusReply += `Wir bearbeiten deine Bestellung. Geschätzte Lieferzeit: ${delivery} Tage.`;
                else
