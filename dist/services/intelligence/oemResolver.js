...
const llmHeuristicSource_1 = require("./sources/llmHeuristicSource");
const partMatchHelper_1 = require("./sources/partMatchHelper");
const baseSource_1 = require("./sources/baseSource");
const backsearch_1 = require("./backsearch");
const motointegratorSource_1 = require("./sources/motointegratorSource");
// import { eBayDuplicateSource } from "./sources/eBayDuplicateSource";
const autodocSource_1 = require("./sources/autodocSource");
const autodocWebSource_1 = require("./sources/autodocWebSource");
const sepZapWebSource_1 = require("./sources/sepZapWebSource");
// NEW ENHANCED SOURCES
const kfzteile24Source_1 = require("./sources/kfzteile24Source");
const pkwteileSource_1 = require("./sources/pkwteileSource");
const openaiVisionSource_1 = require("./sources/openaiVisionSource");
...
const SOURCES = [
    cacheSource_1.cacheSource,
    shopSearchSource_1.shopSearchSource,
    webScrapeSource_1.webScrapeSource,
    llmHeuristicSource_1.llmHeuristicSource,
    motointegratorSource_1.motointegratorSource,
    // eBayDuplicateSource removed (Logic moved to webScrapeSource/oemWebFinder)
    autodocSource_1.autodocSource,
    autodocWebSource_1.autodocWebSource,
    sepZapWebSource_1.sepZapWebSource,
    // NEW HIGH-QUALITY SOURCES
    kfzteile24Source_1.kfzteile24Source,
    pkwteileSource_1.pkwteileSource,
    openaiVisionSource_1.openaiVisionSource
];
...
function mergeCandidates(candidates) {
    ...
    for (const c of candidates) {
        ...
        if (!map.has(key)) {
            ...
            existing.confidence = (0, baseSource_1.clampConfidence)(combined);
            if (c.brand && !existing.brand)
...
function pickPrimary(candidates) {
    if (!candidates.length)
        return { primaryOEM: undefined, note: "Keine OEM-Kandidaten gefunden.", overall: 0 };
...
async function resolveOEM(req) {
    ...
    try {
        const aftermarketCandidates = await (0, partMatchHelper_1.resolveAftermarketToOEM)(req);
    ...
    const filtered = await (0, partMatchHelper_1.filterByPartMatch)(allCandidates, req);
    ...
    for (const c of schemaFiltered) {
        ...
        if (yearMatch)
            c.confidence = (0, baseSource_1.clampConfidence)(c.confidence + 0.05);
        if (kwMatch)
            c.confidence = (0, baseSource_1.clampConfidence)(c.confidence + 0.05);
    }
    ...
    let bestResult = { confidence: 0, note: "" };
    ...
    if (topCandidates.length === 0) {
        ...
        for (const candidate of topCandidates) {
            ...
            try {
                const confirm = await (0, backsearch_1.backsearchOEM)(candidate.oem, req);
                ...
                const validation = await (0, enhancedValidation_1.performEnhancedValidation)(candidate.oem, allCandidates, req.vehicle.make || "", req.vehicle.model || "", req.partQuery.rawText, backsearchResult, {
                ...
                logger_1.logger.warn("OEM validation flow failed", { oem: candidate.oem, error: err?.message });
                currentConf *= 0.5;
 ...
 */
function checkBrandSchema(oem, brand) {
    if (!oem || !brand)
        return 0;
    ...
    if (["VOLKSWAGEN", "AUDI", "SEAT", "SKODA", "VW"].some(b => brand.includes(b))) {
        ...
        if (/^[A-Z0-9]{3}[0-9]{3}[A-Z0-9]{3,6}$/.test(oem))
            return 2;
        if (oem.length >= 9 && oem.length <= 12)
            return 1;
    }
    // BMW / Mini
    if (brand.includes("BMW") || brand.includes("MINI")) {
        ...
        if (digits.length === 11 || digits.length === 7)
            return 2;
        if (oem.length >= 7 && oem.length <= 11)
            return 1;
    }
    // Mercedes / Smart
    if (brand.includes("MERCEDES") || brand.includes("BENZ") || brand.includes("SMART")) {
        // Starts with A, B, W, N ... or just digits
        if (oem.startsWith("A") && oem.length >= 10 && oem.length <= 13)
            return 2;
        if (/^[0-9]{10,12}$/.test(oem))
            return 2;
        if (oem.length >= 10 && oem.length <= 13)
            return 1;
    }
    // Additional brands
    ...
    if (brand.includes("PORSCHE")) {
        if (/^[Pp][0-9]{6,9}$/.test(oem))
            return 2;
        if (/^[A-Z0-9]{7,10}$/.test(oem))
            return 2;
        if (oem.length >= 7 && oem.length <= 12)
            return 1;
    }
    ...
    if (brand.includes("DODGE") || brand.includes("RAM")) {
        if (/^[0-9]{7,8}$/.test(oem))
            return 2;
        if (/^[A-Z0-9]{4}-[A-Z0-9]{4,6}-[A-Z]{1,2}$/.test(oem))
            return 2;
        if (oem.length >= 7 && oem.length <= 15)
            return 1;
    }
    ...
    if (brand.includes("RENAULT") || brand.includes("DACIA") || brand.includes("NISSAN")) {
        ...
        if (/^[0-9R]{10}$/.test(oem))
            return 2; // Renault
        if (/^[A-Z0-9]{5}-[A-Z0-9]{5}$/.test(oem))
            return 2; // Nissan
        if (oem.length >= 8 && oem.length <= 12)
            return 1;
    }
    // FIAT / ALFA / LANCIA
    ...
    if (brand.includes("PEUGEOT") || brand.includes("CITROEN") || brand.includes("PSA")) {
        ...
        if (/^[0-9]{10}$/.test(oem))
            return 2;
        if (/^[A-Z0-9]{4}\.[A-Z0-9]{6}$/.test(oem))
            return 2;
        if (oem.length >= 4 && oem.length <= 12)
            return 1;
    }
    return 0;
}
