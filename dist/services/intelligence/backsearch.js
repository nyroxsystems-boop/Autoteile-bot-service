 ...
 */
async function backsearchOEM(oem, req) {
    const result = {
        tecdocHit: false,
        webHit: false,
        autodocHit: false,
    ...
    const checkCompliance = (html) => {
        const lowerHtml = html.toLowerCase().replace(/[^a-z0-9]/g, "");
        if (!lowerHtml.includes(normalizedOEM))
            return false;
    ...
    tasks.push((async () => {
        try {
            ...
            const res = await (0, node_fetch_1.default)(url, { method: "GET", timeout: 8000 });
            if (res.ok) {
    ...
    if (result.webHit)
        hits++;
    if (result.autodocHit)
        hits++;
    if (result.dapartoHit)
        hits++;
    if (result.ebayHit)
        hits++;
    result.totalHits = hits;
    logger_1.logger.info("Deep Backsearch finished", { oem, hits: result.totalHits });
    return result;
}
