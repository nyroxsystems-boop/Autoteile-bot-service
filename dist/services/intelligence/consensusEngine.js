 ...
 */
function calculateConsensus(candidates, config = {}) {
    ...
    for (const [oem, group] of oemGroups.entries()) {
        ...
        const score = (sourceCount / candidates.length) * (1 - cfg.priorityWeight) +
    ...
    logger_1.logger.info(`[Consensus] Best OEM: ${best.oem} (${best.sourceCount} sources, ${(agreementScore * 100).toFixed(0)}% agreement, ${(confidence * 100).toFixed(0)}% confidence)`);
    return {
        primaryOEM: best.oem,
        confidence,
        agreementScore,
 ...
 */
function validateBrandPattern(oem, brand) {
    const patterns = {
 ...
 */
function applyBrandPatternBoost(result, brand) {
    ...
    if (patternScore >= 0.8) {
        ...
        logger_1.logger.info(`[Consensus] Brand pattern boost: ${result.primaryOEM} matches ${brand} pattern (+5%)`);
        return {
            ...result,
    ...
    if (patternScore <= 0.3) {
        ...
        logger_1.logger.warn(`[Consensus] Brand pattern mismatch: ${result.primaryOEM} doesn't match ${brand} pattern (-10%)`);
        return {
            ...result,
