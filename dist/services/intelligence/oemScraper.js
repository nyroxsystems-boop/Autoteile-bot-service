...
exports.normalizeOem = normalizeOem;
exports.looksLikeOem = looksLikeOem;
exports.extractOemsFromHtml = extractOemsFromHtml;
...
function normalizeOem(input) {
    ...
    if (clean.length < 5 || clean.length > 18)
        return null; // OEMs meist 7-13, minimal 5
    ...
    if (BLACKLIST.has(clean) || BLACKLIST.has(input.trim().toUpperCase()))
        return null;
    // Specific Junk Prefixes
    if (clean.startsWith("ICON") || clean.startsWith("WOFF") || clean.startsWith("FONT"))
        return null;
...
function looksLikeOem(s) {
    return !!normalizeOem(s);
}
function extractOemsFromHtml(html) {
    if (!html)
        return [];
