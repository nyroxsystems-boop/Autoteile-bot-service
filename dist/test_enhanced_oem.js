...
const consensusEngine_1 = require("./services/oemResolver/consensusEngine");
...
async function runTests() {
    ...
    for (const testCase of testCases) {
        ...
        try {
            ...
            const result = await (0, oemResolver_1.resolveOEM)(testCase.request);
            ...
            if (result.primaryOEM && result.overallConfidence >= 0.85 && patternMatch) {
                ...
                console.log(`\n❌ TEST FAILED`);
                if (!result.primaryOEM)
                    console.log(`   Reason: No OEM found`);
                if (result.overallConfidence < 0.85)
                    console.log(`   Reason: Low confidence`);
                if (!patternMatch)
                    console.log(`   Reason: Pattern mismatch`);
            }
        }
        catch (error) {
...
async function testConsensusEngine() {
    ...
    let result = (0, consensusEngine_1.calculateConsensus)(mockCandidates);
    console.log(`\n📤 Consensus Result:`);
    ...
    result = (0, consensusEngine_1.applyBrandPatternBoost)(result, "VW");
