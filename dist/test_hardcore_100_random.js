...
function generateRandomTestCase(index) {
    ...
    if (brandType === 'performance') {
        ...
        partType = Math.random() < 0.6 ? 'common' : Math.random() < 0.85 ? 'exotic' : rand(['suspension', 'electrical']);
    }
...
╚════════════════════════════════════════════════════════════════════════════╝
    ...
    for (let i = 0; i < TEST_CASES.length; i++) {
        ...
        try {
            ...
            const topCandidate = result.candidates.find((c) => c.oem === result.primaryOEM);
    ...
    if (failures.length > 0) {
        ...
        failures.forEach(f => {
            ...
            if (!f.backsearchPassed)
                console.log(`    Backsearch: FAILED (${f.backsearchHits} hits)`);
        });
    }
    // Final verdict
    console.log(`\n${"=".repeat(80)}`);
