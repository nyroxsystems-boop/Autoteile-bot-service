import { OEMSource } from "./sources/baseSource";

// Helper to load resolver with mocked sources
function runWithSources<T>(sources: OEMSource[], fn: (resolve: any) => Promise<T>) {
  jest.resetModules();
  const fallback = (name: string): OEMSource => ({ name, resolveCandidates: async () => [] });
  const pick = (name: string) => sources.find((s) => s.name === name) ?? fallback(name);

  jest.doMock("./sources/cacheSource", () => ({ cacheSource: pick("cache") }));
  jest.doMock("./sources/tecdocLightSource", () => ({ tecdocLightSource: pick("tecdoc_light") }));
  jest.doMock("./sources/tecdocNumberSource", () => ({ tecdocNumberSource: pick("tecdoc_number") }));
  jest.doMock("./sources/tecdocVinRestSource", () => ({ tecdocVinRestSource: pick("tecdoc_vin_rest") }));
  jest.doMock("./sources/shopSearchSource", () => ({ shopSearchSource: pick("shop_search") }));
  jest.doMock("./sources/webScrapeSource", () => ({ webScrapeSource: pick("web_scrape") }));
  jest.doMock("./sources/aiGuessSource", () => ({ aiGuessSource: pick("ai_guess") }));
  jest.doMock("./sources/llmHeuristicSource", () => ({ llmHeuristicSource: pick("llm_heuristic") }));
...
describe("OEM Resolver orchestration", () => {
  ...
  it("happy path with multiple sources agreeing", async () => {
    ...
    await runWithSources(sources, async (resolveOEM: any) => {
      ...
      const srcJoined = res.candidates.find((c: any) => c.oem === "OEM-1")?.source || "";
